...
//     To measure the milk,
//     operations allowed are : -Fill one of the cups completely with milk.- Empty the other cup.- Pour milk from one cup into another cup till it fills completely,
//     or the first cup itself is empty.

//         Such that,
//     at the end one or both cups contains P litres of the milk.

//                       Please help Ramu,
//     to check whether P litres of milk can be measured using the two cups or not .
...
bool check(int m, int n, int p)
{
    ...
    int diff = abs(m - n);
    int c = p % diff;
    return ((a == b) || (a == c) || (b == c) || (a == diff) || (b == diff) || (c == diff));
}
