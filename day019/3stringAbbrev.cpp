...
void findOccur(string s, vector<string> &ans, int idx, int sum, string cur)
{
    if (idx == s.length())
    {
        if (sum > 0)
    ...
    findOccur(s, ans, idx + 1, sum + 1, cur);
    if (sum > 0)
        cur += to_string(sum);
    findOccur(s, ans, idx + 1, 0, cur + s[idx]);
}

vector<string> findOccurences(string s)
{
    vector<string> ans;
    findOccur(s, ans, 0, 0, "");
...
int main()
{
    string w;
    cin >> w;
    vector<string> ans = findOccurences(w);
    // cout<<ans.size()<<endl;
